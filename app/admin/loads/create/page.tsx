...
function AdminCreateLoadPageContent() {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        // Show all drivers (available and on load) - admin can choose any driver
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showApiError(err, 'Failed to create load')
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="px-6 md:px-8 pb-6 md:pb-8 print:p-4">
      ...
      <div className="sticky top-0 z-[55] mb-6 bg-slate-900/95 backdrop-blur-sm -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-4 border-b border-slate-700/50">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-500 bg-clip-text text-transparent mb-2 tracking-tight">Create Load Request</h1>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="glass p-6 rounded-xl mb-6 bg-blue-500/10 border border-blue-500/30">
          <div className="flex items-start gap-3">
            <svg className="w-6 h-6 text-blue-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <h3 className="font-semibold text-blue-300 mb-1">Creating a Load for Shipper</h3>
              ...
              <ul className="text-sm text-slate-300 ml-4 list-disc space-y-1">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, dropoffContactName: e.target.value })}
                    className="w-full px-4 py-3 rounded-lg border border-slate-600/50 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 bg-slate-800/50 text-slate-200 placeholder:text-slate-500"
