...
/**
 * POST /api/load-requests/[id]/calculate-rate-with-deadhead
 * Calculate rate for an existing load including deadhead miles
 */
export async function POST(
  ...
  return withErrorHandling(async (req: NextRequest) => {
    ...
    const { driverStartingLocation, driverId } = data

    if (!driverStartingLocation) {
      return NextResponse.json(
        { error: 'Driver starting location is required' },
        { status: 400 }
      )
    }
    ...
    // Calculate deadhead distance (driver location → pickup)
    const deadheadResult = await calculateDistance(driverStartingLocation, pickupAddress)
    if (!deadheadResult.success) {
      return NextResponse.json(
        {
          error: 'Failed to calculate deadhead distance',
          details: deadheadResult.error,
        },
        { status: 400 }
      )
    }

    const deadheadDistance = deadheadResult.distance
    ...
    const totalDistance = deadheadDistance + loadDistance
    ...
    const ratePerMile = rateResult.breakdown.total / totalDistance
    ...
    await prisma.loadRequest.update({
      where: { id },
      data: {
        deadheadStartingLocation: driverStartingLocation,
        deadheadDistance: deadheadDistance,
        totalDistance: totalDistance,
        ratePerMile: adjustedRatePerMile,
        autoCalculatedDistance: loadDistance, // Store load distance separately
      },
    })
